...
ENV PARAMETERS='template_in_path=/config/mapping.template;template_out_filename=template.json'
...
ENTRYPOINT /protoc/bin/protoc --plugin $ENV_BINARY_NAME --proto_path /config/ --${ENV_BINARY_SUFFIX_NAME}_opt=${PARAMETERS} --${ENV_BINARY_SUFFIX_NAME}_out=/output/ observabilitySchema.proto